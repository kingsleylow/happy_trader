...
public class MainPersistManager {
		...
		// prices are kept in long field multipleed by that value
		public static int PRICE_GENERAL_MULTIPLIER = 10000;
		...
		public void queryPositions(
			...
			int detail_flag
				...
				if (detail_flag == MainPersistManager.QUERY_POSITION_INCLUDE_CHANGE_HISTORY) {
						...
						queryPositions_withDetails(trx, r, accountId, flag_state, openTimeFilter, closeTimeFilter, companyName, serverName);
				} else if (detail_flag == MainPersistManager.QUERY_POSITION_DO_NOT_INCLUDE_CHANGE_HISTORY) {
						queryPositions_noDetails(trx, r, accountId, flag_state, openTimeFilter, closeTimeFilter, companyName, serverName);
				} else {
		...
		public void queryAccountDetails(
			...
			int detail_flag
				...
				if (result == null) {
						// now rows
						throw new HtException(getContext(), "queryAccountDetails", "The request provided no rows");
				}
		...
		private void queryPositions_withDetails(
			...
			String serverName
				...
				if (HtUtils.nvl(serverName) || HtUtils.nvl(companyName) || HtUtils.nvl(accountId))
					throw new HtException(getContext(), "queryPositions_withDetails", "Invalid input data");

				String sql_filter = "";
				if (openTimeFilter >= 0) {
						sql_filter = " and p.open_time >= :open_time_p";
				}

				if (closeTimeFilter >= 0) {
						sql_filter = " and p.close_time >= :close_time_p";
				}
				...
				if (flag_state == MainPersistManager.QUERY_POSITION_ONLY_OPEN || flag_state == MainPersistManager.QUERY_POSITION_ONLY_HISTORY) {
						...
						query = sess.createSQLQuery("select {p.*}, {pup.*} from "
							...
							+ " p.account_minfo_id= am.id and pup.position_id=p.id and am.account_name= :account_name_p and am.server= :server_p "
						...
						throw new HtException(getContext(), "queryPositions_withDetails", "Invalid state flag");
				}

				query.setParameter("account_name_p", accountId);
				query.setParameter("company_name_p", companyName);
				query.setParameter("server_p", serverName);
				...
				for (int i = 0; i < positions.size(); i++) {
						...
						if (pos.getPositionUid() != current_pos_id) {
								...
								pose.getHtMtEventWrapperPositionChange().setCompanyName(companyName);

								// add entry
								//r.getPositions().put(pos.getPositionUid(), pose);
								r.getPositions().add(pose);
		...
		private void queryPositions_noDetails(
			...
			String serverName
				...
				if (HtUtils.nvl(serverName) || HtUtils.nvl(companyName) || HtUtils.nvl(accountId))
					throw new HtException(getContext(), "queryPositions_noDetails", "Invalid input data");
				...
				if (flag_state == MainPersistManager.QUERY_POSITION_ONLY_OPEN || flag_state == MainPersistManager.QUERY_POSITION_ONLY_HISTORY) {
						...
						query = sess.createSQLQuery("select p.* from {h-schema}position p, {h-schema}account_metainfo am, {h-schema}company_metainfo cmi where "
							+ " p.account_minfo_id= am.id and am.account_name= :account_name_p and pos_state = :poststate_p and am.server= :server_p"
							+ " and cmi.id=am.company_minfo_id and cmi.company_name=:company_name_p "
							+ sql_filter
							+ " order by p.id asc");
						...
						query = sess.createSQLQuery("select p.* from {h-schema}position p, {h-schema}account_metainfo am, {h-schema}company_metainfo cmi where "
							+ " p.account_minfo_id=am.id and am.account_name=:account_name_p and am.server= :server_p"
							+ " and cmi.id=am.company_minfo_id and cmi.company_name=:company_name_p "
							+ sql_filter
							+ " order by p.id asc");
				} else {
