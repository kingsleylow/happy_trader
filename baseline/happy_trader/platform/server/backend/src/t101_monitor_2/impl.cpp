...
#define T101 "T101"
#define PROVIDER "Broko MT Provider"
...
// user tags
#define CUR_POS_TAG tag_m
#define TYPE_POS tag2_m
...
// -------------------------------
	...
	map<int, CppUtils::String> Symbol::symbolIdentifiersReverse_m = map<int, CppUtils::String>();
...
int BootManager::boot ()
{
	...
	for(map<CppUtils::String, pair<int, bool> >::const_iterator it = Symbol::symbolIdentifiers_m.begin(); it != Symbol::symbolIdentifiers_m.end(); it++) {
		Symbol::symbolIdentifiersReverse_m[it->second.first] = it->first;
	}
...
// Symbol
	Symbol::Symbol(
		...
		CppUtils::String Symbol::getSymbolName(int const id)
		{
			map<int, CppUtils::String>::const_iterator it = symbolIdentifiersReverse_m.find(id);
			{
				LOCK_FOR_SCOPE(Symbol::symbolIdentifiersMtx_m);

				if (it != symbolIdentifiersReverse_m.end())
					return it->second;
				else {
					THROW(CppUtils::OperationFailed, "exc_SymbolNotRegistered", "ctx_getSymbolName", (long)id );
				}
			}
...
)
{
			...
			for(multimap<double, Symbol>::const_iterator it = secondOne.sortedSymbols_m.begin(); it != secondOne.sortedSymbols_m.end(); it++) {
				...
				if (CppUtils::contains(changes, it->second.id_m)) {
					...
					// save
					Signal signal(Signal::ST_NotifyCurPos, it->second.symbol_m, it->second.id_m, idx );
					...
					if (bit->second <= zero1) {
						...
						if(ait != afterIds.end()) {
							if (ait->second >= zero2) {
								...
								Signal signal(Signal::ST_ZeroDown, Symbol::getSymbolName(bit->first), bit->first, ait->second );
								signallist.push_back(signal);
						...
						if(ait != afterIds.end()) {
							if (ait->second <= zero1) {
								...
								Signal signal(Signal::ST_ZeroUp, Symbol::getSymbolName(bit->first), bit->first, ait->second );
								signallist.push_back(signal);
	...
	)
	{
			...
			for( int i = 0; i < shortSymbols.size(); i++) {
				mInfo.ID_m.generate();
				initFromFullName(shortSymbols[i], mInfo);
				oper.unblockingIssueOrder( mInfo);
			}

			for( int i = 0; i < longSymbols.size(); i++) {
				mInfo.ID_m.generate();
				initFromFullName(longSymbols[i], mInfo);
				oper.unblockingIssueOrder( mInfo);
			}
		...
		)
	{
		...
		*/  
		...
		// do nothing before our cut off time
		//if (milisecs_time_m > rtdata.getTime() )
		//	return;

		checkPosTick(rtdata, brokerlib);
		...
		if (executePeriod_m != Inqueue::AP_Dummy) {
			...
			if (cache_exec_period_id > 0) {
				...
				//CppUtils::String dump_str = historyaccessor.dump(cache_exec_period_id);
				//CppUtils::saveContentInFile2("c:\\temp_foo\\content_"+rtdata.getSymbol2()+".txt", dump_str);
				...
				if (unclosed.isNewPeriod()) {
					...
					set<CppUtils::String>::iterator it = periodFinishSet_m.find(rtdata.getSymbol2());
					if (it == periodFinishSet_m.end()) {
						periodFinishSet_m.insert(rtdata.getSymbol2());
						//LOGEVENT(HlpStruct::CommonLog::LL_INFO,  T101, "Inserted into pswitcher " << rtdata.getSymbol2() << " size " << 
						//	periodFinishSet_m.size() << " RT data " << rtdata.toString());
					}
		...
		if (!isReady_m) {
			...
			for( int i = 0; i < shortSymbols.size(); i++) {
				calculateFirstPrice(rtdata, milisecs_time_m, price, rtdata.getProvider(), shortSymbols[i], historyaccessor );
			...
			for( int i = 0; i < longSymbols.size(); i++) {
				calculateFirstPrice(rtdata, milisecs_time_m, price, rtdata.getProvider(), longSymbols[i], historyaccessor );
			...
			isReady_m = true;
			LOGEVENT(HlpStruct::CommonLog::LL_INFO,  T101, "Ready to work... whereas RT " << rtdata.toString());
			
			ALERTEVENTNC(runName, BrkLib::AP_HIGH, "T101 actually ready to work", "T101 actually ready to work... whereas RT " << rtdata.toString() );
				...
				if (basketList_m.size() > 0 ) {
					...
					if (!newEntry.basket_m.isNormalSize()) {
						...
						if (event & BasketStructure::E_Changed) {
							...
							// it is also trading function
							if (milisecs_time_m <= rtdata.getTime()) {

								vector<Signal> signallist;
								sendAlert(rtdata.getTime(), "", newEntry, topEntry, runName, signallist);
								doTrade(rtdata, brokerlib, signallist);
							}
	...
	{
		static const CppUtils::String name("t101 indicator algorithm - trade 2");
		return name;
	}
		...
		Inqueue::ContextAccessor& historyaccessor)
	{
			...
			if (!found) {
				if (alertedCounter_m >= 1000) {
					LOGEVENT(HlpStruct::CommonLog::LL_ERROR,  T101, "Can't find past price data for " << symbol << CppUtils::getGmtTime(timestamp) 
						<< " whereas RT " << rtdata.toString());
					alertedCounter_m = 0;
				}
				alertedCounter_m++;
				return;
	...
	)
	{
		...
		BasketStructure::toHTML2Baskets(
			oldEntry.basket_m, 
			newEntry.basket_m, 
			newEntry.changes_m, 
			priority, 
		...
		// here we will have trading functions
	}

	void AlgorithT101Indic::checkPosTick(HlpStruct::RtData const& rtdata, BrkLib::BrokerBase* brokerlib)
	{
		// check positions
	}

	void AlgorithT101Indic::doTrade(HlpStruct::RtData const& rtdata, BrkLib::BrokerBase* brokerlib, vector<Signal>& signallist)
	{
		// for open positions must change some parameters
		BrkLib::BrokerOperation oper(brokerConnect_m);
		BrkLib::PositionList const& poslist = oper.getBrokerPositionList(BrkLib::QUERY_OPEN);

		
		// signals
		for(int i = 0; i < signallist.size(); i++) {
			Signal const& signal_i = signallist[i];


			bool skip_that_signal = false;
			for(int i = 0; i < poslist.size(); i++) {
				BrkLib::Position const& pos_i = poslist[i];

				if (pos_i.TYPE_POS < 0) {
					LOGEVENT(HlpStruct::CommonLog::LL_ERROR,  T101, "Invalid position tag2_m");
					return;
				}

				if (pos_i.CUR_POS_TAG < 0) {
					LOGEVENT(HlpStruct::CommonLog::LL_ERROR,  T101, "Invalid position tag_m");
					return;
				}

				if ((pos_i.symbolNum_m+pos_i.symbolDenom_m) == signal_i.symbol_m	) {

					// for that position and symbol skip further processing
					skip_that_signal = true;

					if (signal_i.type_m == Signal::ST_NotifyCurPos) {

						// save new position check if need to close and return as no need to do any thing
						if (pos_i.TYPE_POS == (int)Signal::ST_ZeroDown) {
							// должно увеличение индекса

							if (signal_i.curPos_m > pos_i.CUR_POS_TAG) {
								oper.updatePositionTagFields(pos_i.ID_m, signal_i.curPos_m, pos_i.tag2_m, BrkLib::QUERY_OPEN  );
								
							}
							else {
								// не пора ли закрыть позицию
								if (signal_i.curPos_m <= (pos_i.CUR_POS_TAG-2)) {
									// close
									BrkLib::Order order;
									BrkLib::initCloseOrder(order, rtdata.getProvider(), signal_i.symbolNum_m, signal_i.symbolDenom_m, pos_i.brokerPositionID_m, rtdata.getTime());
						...
						else if(pos_i.TYPE_POS == (int)Signal::ST_ZeroUp) {
							// должно уменьшение индекса
							if (signal_i.curPos_m < pos_i.CUR_POS_TAG) {
								oper.updatePositionTagFields(pos_i.ID_m, signal_i.curPos_m, pos_i.tag2_m, BrkLib::QUERY_OPEN );
							}
							else {
					...
					} // signal_i.type_m == Signal::ST_NotifyCurPos
				
				} // pos_i.symbolNum_m+pos_i.symbolDenom_m) == signal_i.symbol_m	
				
			}

		
			// if that we have that symbol for opened pos we need to continue the external loop
			if (skip_that_signal)
				continue;

			//if (signal_i.symbol_m!="CADJPY" && signal_i.symbol_m!="GBPJPY")
			//	continue;
			...
			bool is_buy;
			if (signal_i.type_m == Signal::ST_ZeroUp) {
			
				  
				
				// THAT IS CORRECT
				//is_buy = Symbol::isLong(signal_i.symbol_m) ? false:true;

				// VISA_VERSA
				is_buy = Symbol::isLong(signal_i.symbol_m) ? true:false;

				// init market order
				BrkLib::initMarketOrder(order, rtdata.getProvider(), signal_i.symbolNum_m, signal_i.symbolDenom_m, is_buy, -1,-1,-1,100000,-1, rtdata.getTime() );
				
				// initial current position
				order.CUR_POS_TAG = signal_i.curPos_m;
				order.TYPE_POS = signal_i.type_m;

				oper.unblockingIssueOrder( order );

			}
			else if (signal_i.type_m == Signal::ST_ZeroDown) {
