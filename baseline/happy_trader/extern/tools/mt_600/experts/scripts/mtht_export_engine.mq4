...
#import "htmtproxy_bck.dll"
  ...
  int connectToPipeServer();
	...
	int getPollingQueueNextItem( int tout_msec, string& user[], string& password[], string& server[]);
...
bool makeExport(  string mt_uid)
{
   ...
   if (result[0] != RETURN_MT_DATA_ACCEPTED) {
      ...
      Print("Pipe server returned error: [", result[0], "], exiting...");
			return (false );
   }
...
string getTerminalUid_W()
{
	...
	terminal_uid[0] = "00000000000000000000000000000000";
  getTerminalUid( terminal_uid );

	return (terminal_uid[0]);
}
  ...
  */
  ...
  while(!IsStopped()) {
      ...
      // read accounts data
		  ...
		  /*
      string user[1], password[1], server[1];
			 
			user[0] =			"r                                                                  ";
			password[0] = "p                                                                  ";
			server[0] =		"s                                                                  ";

      bool result = getPollingQueueNextItem( 2*1000, user, password, server);
			*/

			string raw_data[1];
			raw_data[0] = initString(25);
      ...
      // wait for data availabe for 10 seconds
         ...
         return(0);
			...
			Print("Raw command obtained: ", raw_data[0]);
			  
			string user_i, password_i, server_i, lifetime_i, order_info_i;
			int parse_result = processReturnedComand(raw_data[0],user_i, password_i, server_i, lifetime_i, order_info_i );
