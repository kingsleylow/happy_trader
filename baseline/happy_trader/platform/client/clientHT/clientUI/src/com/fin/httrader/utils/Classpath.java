...
public class Classpath {
   ...
   */
  public static List<String> to_jvm_name (String package_name, File [] files) {
      
    List<String> result = new ArrayList<String>();
    for (int i=0; i<files.length; i++) {
			if (files[i].isFile()) {
				result.add( to_jvm_name (package_name, files [i]) );
			}
			else {
				// this is a directory
				String new_package_name = package_name + "." + files[i].getName();

				// get list of files
				File nested_dir = find_file(to_filename(new_package_name));
				List<String> nested = to_jvm_name(new_package_name, nested_dir.listFiles());
